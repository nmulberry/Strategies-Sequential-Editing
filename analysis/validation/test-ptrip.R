...
test_p_over_trees <- function(n, beta, ell){
    ...
    ## run sims
    pars <- crossing(nsim=100, k=c(1,5, 13), lambda = seq(1,30, by=1), m=c(10,50),ell=ell, j=c(4,64))
    ...
    res_summ$dmax <- 1-res_summ$ell
    res_summ$p0 <- res_summ %>% pmap_dbl(., ~pfull_0(..5,..1,..4,..2,..8,..9,..10))
    res_summ$p1 <- res_summ %>% pmap_dbl(., ~pfull_1(..5,..1,..4,..2,..8,..9,..10))
    res_summ$beta <- beta
    return(res_summ)
}
...
pars <- crossing(n=c(4), beta=c(5, 10), ell=c(0.001)) 
res <- pars %>% pmap_dfr(test_p_over_trees)
res <- res %>% rename("B1"=p1, "B0"=p0, "Simulated"=upgma_score)
res <- res %>% pivot_longer(cols=c(B1,B0,Simulated))
res$k <- factor(res$k)
res$name <- factor(res$name, levels=c("Simulated", "B1", "B0"))
gg <- ggplot(res, aes(x=lambda, y=value, col=name, linetype=name))+geom_line(linewidth=1.1)+
    facet_grid(m+beta~j+k, labeller=label_both)+
    labs(y="Reconstruction Probability", x=expression(lambda), col="", linetype="")+
    scale_colour_manual(values=c("B1"="red", "B0"="lightblue", "Simulated"="black"))+
    scale_linetype_manual(values=c("B1"="dashed", "B0"="dashed", "Simulated"="solid"))+
    theme(legend.position="bottom")

ggsave("all_test_trees.pdf", width=10, height=9)
